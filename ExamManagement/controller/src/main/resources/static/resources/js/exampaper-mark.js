...
//			this.bindAddQustionToPaper();
		...
		loadAnswerSheet : function loadAnswerSheet(){
			$.ajax({
				...
				type : "GET",
				async : false,
				url : "/Management/exam/get-answersheet/" + $("#hist-id").val(),
		...
		bindFinish : function bindFinish(){
			...
			btn.click(function(){
				...
				var request = $.ajax({
					...
					type : "POST",
					url :  "/Management/exam/answersheet",
				...
				request.done(function(message, tst, jqXHR) {
					...
					if (message.result == "success") {
						...
						util.success("阅卷完成！", function() {
							window.location.replace(document.getElementsByTagName('base')[0].href + util.getCurrentRole() + '/exam-student-list/' + $("#exam-id").val());
						});
					} else {
