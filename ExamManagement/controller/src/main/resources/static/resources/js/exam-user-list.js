		...
		bindAddGroupUser : function bindAddGroupUser(){
			$("#link-user-group-btn").click(function(){
				...
				$.ajax({
					...
					type : "POST",
					url : "/Management/exam/add-exam-group/" + $("#exam-id-hidden").val(),
		...
		bindAddUser : function bindAddUser(){
			$("#link-user-btn").click(function(){
				...
				jQuery.ajax({
					...
					type : "POST",
					url : "/Management/exam/add-exam-user/" + $("#exam-id-hidden").val(),
					data : data,
					success : function(message, tst, jqXHR) {
						if (message.result == "success") {
		...
		bindApprove : function bindApprove(){
			$(".approved-btn").click(function(){
				...
				$.ajax({
					...
					type : "GET",
					url : "/Management/exam/mark-hist/" + $(this).data("id") + "/1",
					success : function(message, tst, jqXHR) {
						if (!util.checkSessionOut(jqXHR))
							return false;
						if (message.result == "success") {
		...
		bindDisapprove : function bindDisapprove(){
			$(".disapproved-btn").click(function(){
				...
				$.ajax({
					...
					type : "GET",
					url :  "/Management/exam/delete-hist/" + $(this).data("id"),
					success : function(message, tst, jqXHR) {
						if (!util.checkSessionOut(jqXHR))
							return false;
						if (message.result == "success") {
		...
		searchPaper : function searchPaper(){
			$("#btn-search").click(function(){
				...
				if(searchStr != ""){
					...
					+ util.getCurrentRole() + "/exam-student-list/" + $("#exam-id-hidden").val() + "?" + paramStr;
				}
				else
					document.location.href = document.getElementsByTagName('base')[0].href
					+ util.getCurrentRole() + "/exam-student-list/" + $("#exam-id-hidden").val() + "?" + params;
			});
		},
