			...
			btn.click(function(){
				...
				if(count!=  $(".question").length){
					...
					var request = $.ajax({
						...
						type : "POST",
						url : '/Management/exampaper/update-exampaper/' + $("#exampaper-id").text(),
					...
					request.done(function(message,tst,jqXHR) {
						...
						if (message.result == "success") {
							util.success("修改成功", function() {
								document.location.href = document.getElementsByTagName('base')[0].href + util.getCurrentRole() + '/exampaper-list/0';
							});
						}else {
			...
			$("button#add-list-to-exampaper").click(function() {
				...
				if (checkboxs.length == 0) {
					...
					var request = $.ajax({
						...
						type : "POST",
						url : util.getCurrentRole() + '/get-question-detail4add',
