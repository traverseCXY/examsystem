		...
		bindApprove : function bindApprove(){
			$(".approved-btn").click(function(){
				$.ajax({
					...
					type : "GET",
					url :  "/Management/exam/mark-exam/" + $(this).data("id") + "/1",
		...
		bindAddUser : function bindAddUser(){
			$("#link-user-btn").click(function(){
				...
				jQuery.ajax({
					...
					type : "POST",
					url :  "/Management/exam/add-exam-user/" + $("#link-user-btn").data("id"),
					data : data,
					success : function(message, tst, jqXHR) {
						if (message.result == "success") {
		...
		bindDisapprove : function bindDisapprove(){
			$(".disapproved-btn").click(function(){
				...
				$.ajax({
					...
					type : "GET",
					url : "/Management/exam/mark-exam/" + $(this).data("id") + "/2",
					success : function(message, tst, jqXHR) {
						if (!util.checkSessionOut(jqXHR))
							return false;
						if (message.result == "success") {
		...
		bindDelete : function bindDelete(){
			$(".delete-btn").click(function(){
				...
				$.ajax({
					...
					type : "GET",
					url : "/Management/exam/delete-exam/" + $(this).data("id"),
					success : function(message, tst, jqXHR) {
						if (!util.checkSessionOut(jqXHR))
							return false;
						if (message.result == "success") {
