											...
											<td>
												<c:choose>
													<c:when test="${item.approved == 1 }">
														<a href="/Portal/student/exam-start/${item.examId }" class="btn btn-success btn-sm join-practice-btn">参加考试</a>
													</c:when>
													<c:when test="${item.approved == 3 }">
														<a href="/Portal/student/student-answer-sheet/${item.examId }" class="btn btn-success btn-sm join-practice-btn">查看详情</a>
													</c:when>
								...
								<tbody>
									<c:forEach items="${modelTestToStart }" var="item">
										<tr>
											...
											<td><span class="span-success question-number-2"><fmt:formatDate value="${item.expTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></td>
											<td>
												<a href="/Portal/student/model-test-start/${item.examId }" class="btn btn-success btn-sm join-practice-btn">参加考试</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
			...
			$(function(){
				$(".apply-exam-btn").click(function(){
					...
					jQuery.ajax({
						...
						type : "GET",
						url : "/Portal/student/exam/send-apply/" + examId,
